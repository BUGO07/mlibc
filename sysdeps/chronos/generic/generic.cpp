...
void sys_libc_log([[maybe_unused]] const char *message) {
    // chronos_syscall(SYSCALL_DEBUG, (uint64_t) message, strlen(message));
}

[[noreturn]] void sys_libc_panic() {
    // chronos_syscall(SYSCALL_EXIT, 0, false);
    __builtin_unreachable();
}

[[noreturn]] void sys_exit([[maybe_unused]] int status) {
    // chronos_syscall(SYSCALL_EXIT, status, false);
    __builtin_unreachable();
}

int sys_tcb_set([[maybe_unused]] void *pointer) {
    // chronos_syscall(SYSCALL_SET_TCB, (uintptr_t) pointer);
    return 0;
}
